...
#
# Description: Phonebook app function library
#
...
# catch if user clicks on upper-right 'X' and ensure they want to close
def ask_quit(self):
...
def onSelect(self,event):
    ...
    with conn:
        ...
        # This returns a tuple which we can slice into 4 parts using data[] during the iteration
        for data in varBody:
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # remove any blank space before and after the user's entry
    var_lname = var_lname.strip() 
    var_fname = var_fname.title() # ensure that the first character in each first and last name is capitalized
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname,var_lname)) # combine our normailzed names into a full name
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0):
        ...
        if not "@" or not "." in var_email:
            messagebox.showerror("Email format error","Please ensure that the email address is properly formatted.\nExample: name@domain.net")
        with conn:
            cur = conn.cursor()
            # Check database for existence of the fullname, if so we will alert user and disregard request
            ...
            if chkName == 0:
                ...
                onClear(self) # call function to clear all text boxes
            else:
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) # call function to clear all of the text boxes and the selected index of listbox
...
def onRefresh(self):
    ...
    with conn:
        ...
        i = 0
        while i < count:
            cursor.execute("""SELECT col_fullname FROM tbl_phonebook ORDER BY col_lname DESC""")
...
def onUpdate(self):
    try:
        ...
        return
    # The user will only be allowed to make changes to phone numbers and emails.
